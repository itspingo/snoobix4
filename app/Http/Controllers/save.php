...
class save extends Controller{
        ...
        if(strlen($sfile_name) > 2){
            ...
            for($i=0; $i < count($arsfiles); $i++){
              if($req->hasfile($arsfiles[$i])){
                ...
                $upldfile = $req->file($arsfiles[$i])->store('uploads');
                //dd($upldfile);
                if($upldfile){
                    $upldata[$arsfiles[$i]] = str_replace('uploads/','',$upldfile);
              ...
              if($req->hasfile($arsfiles[$i])){
                ...
                //dd( $upldfile);
                 if(strlen($upldfiles)>20){
                        $mupldata[$arsfiles[$i]] =  str_replace('uploads/','',$upldfiles); 
                        $fileaved = DB::table($mdltblname)
                        //                 //->selectRaw('*')
                                        ->where('id', '=', $rid)
                        //                 //->groupBy('status')
    ...
    //return \Redirect::route('/backend/module',['mid'=>$mid])->with('msg_success','Record is saved successfuly');
    return redirect('/add?mid='.$mid)->with('msg_success','Record is saved successfuly');;
    }
}